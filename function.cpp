//
// Created by Divyansh Garg on 2/25/2020.
//
...
void redirect_cmd(char** cmd, char** file) {
    ...
    if (fork() == 0) {
        ...
        // function call here, I got my Minishell prompt repeated over and over
...
void run_cmd(int argc, char** argv) {
    ...
    // pid should not be below 0. This means there is an error
    if (pid < 0){
        perror("Error (pid < 0)");
    }
        // child process
...
bool to_quit(string choice) {
    // change to users input to lowercase
